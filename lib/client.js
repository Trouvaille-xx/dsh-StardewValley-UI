window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var ostPref = { on: true, file: 'bgm--08_Pelican_Town.mp3' };
    var lastClick = 0;
    ...
    var WEATHERS = {
      ...
      'light-rain': { label: '小雨', badge: '☂ RAINING', badgeColor: '#7FA8C9', tint: 'rgba(26,48,66,0.12)', rain: { density: 0.85, opacity: 0.3 }, audio: ['bgm--rain.wav', 0.55] },
      'heavy-rain': { label: '大雨', badge: '☂ RAIN HEAVY', badgeColor: '#5B86A8', tint: 'rgba(14,32,48,0.22)', rain: { density: 1.4, opacity: 0.48 }, audio: ['bgm--rain.wav', 0.72] },
      'light-snow': { label: '小雪', badge: '❄ SNOWFALL', badgeColor: '#CFE3F2', tint: 'rgba(205,220,240,0.1)', snow: { density: 0.9, opacity: 0.35 }, audio: ['bgm--winter_day.wav', 0.42] },
      'heavy-snow': { label: '大雪', badge: '❄ SNOW STORM', badgeColor: '#DCEAF5', tint: 'rgba(215,228,242,0.16)', snow: { density: 1.6, opacity: 0.5 }, audio: ['bgm--winter_day.wav', 0.58] }
    };
    ...
    function apply(ctx) {
      ...
      function Page() {
        ...
        var pf = React.useState(false), pixelFont = pf[0], setPixelFont = pf[1];
        ...
        return el('div', { className: 'svp-page' }, [
          ...
          Section('3 · 背景 OST', '完整星露谷原声。默认开启并循环播放所选曲目（bgm--08 Pelican Town）；点击曲目可切换/试听，关闭开关即停止。', el('div', null, [
          ...
          Section('4 · 像素字体启用', '默认关闭（保持 DSH 默认字体）；开启后全局使用 Fusion Pixel SC / Silkscreen 像素字体（需自备字体文件）。', el('div', { className: 'svp-row' }, [pixelToggle, el('span', { className: 'svp-note' }, pixelFont ? '已启用' : '未启用')])),
